...
cloud-connector/
  ...
  .goreleaser.yaml     -- Release config: Linux/Windows binaries (386/amd64/arm64), macOS binaries (amd64/arm64) + Docker image
...
- **Release targets**: GoReleaser builds Linux and Windows binaries for `386`, `amd64`, and `arm64`, plus macOS binaries for `amd64` and `arm64`. Docker images still use the Linux `amd64` artifact.
